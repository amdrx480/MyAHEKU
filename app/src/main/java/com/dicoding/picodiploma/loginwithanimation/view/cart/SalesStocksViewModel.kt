...
import com.dicoding.picodiploma.loginwithanimation.service.data.sales.*
...
class SalesStocksViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    // CartActivity
    private val _cartItems = MutableLiveData<List<ListCartItems>>()
    ...
    val itemStock: LiveData<List<ListStocksItem>> = _itemStock
    ...
    private val _selectedItemStock = MutableLiveData<ListStocksItem?>()
    val selectedItemStock: LiveData<ListStocksItem?> = _selectedItemStock
    fun setSelectedStockItem(stock: ListStocksItem?) {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    if (responseBody != null && !responseBody.error) {
                        ...
                        _customersList.value = responseBody.token
...
//                        }
    ...
    fun showListStock(token: String) {
        ...
        client.enqueue(object : Callback<AllStocksResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    if (responseBody != null) {
                        if (!responseBody.error) {
                            _itemStock.value = responseBody.token
